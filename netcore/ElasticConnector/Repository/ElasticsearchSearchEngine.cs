...
namespace ElasticConnector.Repository
{
        ...
        where TEntity : class
    {
        ...
        {
            var mapping = this.client.GetMapping<TEntity>(d => d.Index(this.currentIndex).Type(this.currentType));
        ...
        public void Save(TEntity entity)
        {
            ...
            this.client.Index(entity, d => d.Index(this.currentIndex).Type(this.currentType));
        }
        ...
        public void SaveMany(IEnumerable<TEntity> entites)
        {
            ...
            this.client.IndexMany(entites, index: this.currentIndex, type: typeof(TEntity).Name);
            this.client.Refresh(Indices.All);
        }
        ...
        public void SaveMany(Dictionary<object, TEntity> entites)
        {
            ...
            foreach (var item in entites)
            {
                ...
                operation.Index = this.currentIndex;
                operation.Type = this.currentType;
        ...
        public void SwapAlias(string oldIndex, string newIndex)
        {
            if (!string.IsNullOrEmpty(oldIndex))
                this.client.Alias(alias =>
                {
                    ...
                    if (this.client.IndexExists(oldIndex).Exists)
                    {
                        alias.Remove(c => c.Index(oldIndex).Alias(this.aliasName));
                    }
                    return alias;
                });
            else
                this.client.Alias(a => a.Add(c => c.Index(newIndex).Alias(this.aliasName)));
        }
